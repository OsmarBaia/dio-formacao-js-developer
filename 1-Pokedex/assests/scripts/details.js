...
function StyledDetailsContainer(data) {
    ...
    document.querySelector('body').style.backgroundColor = `var(${colors[1]})`;
...
function PokemonDescription(pokemonData) {
    const pokemonTypes = pokemonData.types
        .map((element) => {
            return `<span class="pokemon-type px-3 py-1 m-1 rounded-4 text-center fw-bold">${element.type.name}</span>`;
        })
        .join("");
    ...
    return ` 
        <div class="pokemon-preview w-100 d-flex flex-column align-items-center">
            ...
            <div class="w-100 px-2 pokemon-types">
                    ${pokemonTypes}
            </div>
            <img src="https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/other/dream-world/${
        pokemonData.id}.svg" alt="Ilustração ${pokemonData.name}" class="pokemon-image img-fluid p-1 mt-4 mb-2 mx-auto">
       </div>
    `;
}
...
function PokemonStats(pokemonData) {
    const pokemonStats = pokemonData.stats
        .map((element) => {
            return `       <div class="row row-cols-12 w-100 d-flex align-items-center justify-content-between">
                            <div class="col-6 text-capitalize">${element.stat.name}</div>
                            <div class="col-2 fw-bold">${element.base_stat}</div>
                            <div class="col-4">
                                <div class="progress w-100 ms-2">
                                    <div class="progress-bar bg-blue" role="progressbar" style="width: ${
                (element.base_stat / 154) * 100
            }%"
                                        aria-valuenow="${element.base_stat}" 
                                        aria-valuemin="0" aria-valuemax="154">
                                    </div>
                                </div>
                            </div>
                        </div>`;
